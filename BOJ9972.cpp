...
void testCase(void) {
    ...
    for (int i = 0; i < 3 && isSolved; i++) {
        for (int j = 0; j < 4 && isSolved; j++) {
            const int startR = i * 3;
            const int endR = (i + 1) * 3;
            const int startC = j * 3;
            const int endC = (j + 1) * 3;
            
            for (int k = startR; k < endR && isSolved; k++) {
                for (int l = startC; l < endC && isSolved; l++) {
                    if (cube[startR][startC] != cube[k][l]) {
                        isSolved = false;
                    }
                }
            }
